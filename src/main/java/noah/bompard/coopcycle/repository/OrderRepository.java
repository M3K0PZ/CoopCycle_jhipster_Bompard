...
public interface OrderRepository extends ReactiveCrudRepository<Order, Long>, OrderRepositoryInternal {
    ...
    @Query("SELECT * FROM jhi_order entity WHERE entity.restaurant_id = :id")
    Flux<Order> findByRestaurant(Long id);

    @Query("SELECT * FROM jhi_order entity WHERE entity.restaurant_id IS NULL")
    Flux<Order> findAllWhereRestaurantIsNull();

    @Query("SELECT * FROM jhi_order entity WHERE entity.id not in (select basket_id from basket)")
    Flux<Order> findAllWhereBasketIsNull();
